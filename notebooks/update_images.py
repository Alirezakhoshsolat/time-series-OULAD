...
try:
    ...
    if 'cells' in notebook_content:
            ...
            if cell['cell_type'] == 'markdown':
                for j, source in enumerate(cell['source']):
                    ...
                    if 'Plotting Anomalies.png' in source:
                        # Since we don't have this specific image, use a placeholder message
                        cell['source'][j] = source.replace(
                            'Plotting Anomalies.png', 
                            'Note: The "Plotting Anomalies.png" visualization is not available, but other related visualizations are in the images folder.'
                        )
                        modified = True
                        print(f"Updated missing 'Plotting Anomalies.png' reference in cell {i+1}")
        ...
        if modified:
            ...
            # Rename files
            os.rename(notebook_path, notebook_path + '.bak2')
            os.rename(notebook_path + '.fixed', notebook_path)
            print(f"Original notebook renamed to {notebook_path}.bak2")
